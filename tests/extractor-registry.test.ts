...
describe('ExtractorRegistry', () => {
	...
	describe('findExtractor', () => {
		...
		test('should cache extractor results', () => {
			...
			expect(extractor1).toBeTruthy();
			expect(extractor2).toBeTruthy();
		});
		...
		test('should handle invalid URLs gracefully', () => {
			...
			// The extractor registry should handle invalid URLs gracefully
			let extractor = null;
			try {
				extractor = ExtractorRegistry.findExtractor(doc, 'not-a-url', {});
				// Should return null for invalid URLs
				expect(extractor).toBeNull();
			} catch (e) {
				// If it throws, that's also acceptable - the test verifies graceful handling
				expect(e).toBeTruthy();
			}
		});
	});
